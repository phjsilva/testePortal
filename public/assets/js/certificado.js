...
(function () {
    ...
    function criarCaminhoOnduladoSelo(cx, cy, r, numScallops, depth) {
        ...
        for (let i = 0; i < numScallops; i++) {
            ...
            path.push(
                `Q ${xPeak.toFixed(2)} ${yPeak.toFixed(2)} ${xVal2.toFixed(2)} ${yVal2.toFixed(2)}`,
            );
        }
        path.push("Z");
        return path.join(" ");
    ...
    function popularTabelaNotas(modulosConcluidos, mediaFinal) {
        ...
        modulosConcluidos.forEach((modulo) => {
            ...
            const notas = modulo.notasTentativas.map((t) =>
                Number(t.nota || 0),
            );
            const melhorNota = notas.length > 0 ? Math.max(...notas) : 0;

            const cell = document.querySelector(
                `[data-grade-modulo-${idModulo}]`,
            );
            if (cell) {
    ...
    function popularCamposDoCertificado(data) {
        ...
        document.querySelector("[data-certificate-name]").textContent =
            data.aluno.nome;
        document.querySelector("[data-certificate-cpf]").textContent =
            formatarCPF(data.aluno.cpf);
        ...
        if (scallopPathEl) {
            scallopPathEl.setAttribute(
                "d",
                criarCaminhoOnduladoSelo(80, 80, 60, 42, 4.5),
            );
        }
    ...
    function exportarCertificadoParaPDF() {
        ...
        const elementoCertificado = document.getElementById(
            "certificate-content",
        );
        ...
        html2canvas(elementoCertificado, {
            ...
            height: 778,
            onclone: (clonedDoc) => {
                const clonedWrapper = clonedDoc.getElementById(
                    "certificate-wrapper",
                );
                const clonedCert = clonedDoc.getElementById(
                    "certificate-content",
                );
                if (clonedWrapper) {
    ...
    document.addEventListener("DOMContentLoaded", async () => {
        ...
        const certificateWrapper = document.getElementById(
            "certificate-wrapper",
        );
        ...
        if (!hash) {
            ...
            errorMessageEl.textContent =
                "Nenhum código identificador de certificado foi fornecido.";
            return;
        }
        ...
        try {
            ...
            document
                ...
                .addEventListener("click", exportarCertificadoParaPDF);
        } catch (error) {
