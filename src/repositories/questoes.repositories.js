...
async function usuarioConcluiuExame(idExame) {
    ...
    return row.total === QUESTOES_POR_TENTATIVA && row.respondidas >= row.total;
}
...
async function criarExameModulo(usuarioId, idModulo) {
    ...
    const aprovadoResult = await pool.query(
        `
      SELECT BOOL_OR(concluida AND nota >= ${NOTA_MINIMA_APROVACAO}) AS aprovado
      FROM (
        SELECT
    ...
    if (aprovadoResult.rows[0]?.aprovado) {
    ...
    if (tentativasExistentes >= MAX_TENTATIVAS) {
        const error = new Error(
            "Limite de 2 tentativas atingido para este modulo.",
        );
        error.statusCode = 409;
        throw error;
    }
